...
public abstract class ClusterClient {
    ...
    public JobSubmissionResult run(PackagedProgram prog, int parallelism) throws ProgramInvocationException, ProgramMissingJobException {
        ...
        if (prog.isUsingProgramEntryPoint()) {
            ...
            ContextEnvironmentFactory factory = new ContextEnvironmentFactory(this, libraries, prog.getClasspaths(), prog.getUserCodeClassLoader(), parallelism, isDetached(), prog.getSavepointSettings());
    ...
    public ActorGateway getJobManagerGateway() throws Exception {
        ...
        return LeaderRetrievalUtils.retrieveLeaderGateway(LeaderRetrievalUtils.createLeaderRetrievalService(flinkConfig), actorSystemLoader.get(), lookupTimeout);
    }
