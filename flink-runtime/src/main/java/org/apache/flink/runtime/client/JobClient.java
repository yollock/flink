...
public class JobClient {
    ...
    public static JobExecutionResult awaitJobResult(JobListeningContext listeningContext) throws JobExecutionException {
        ...
        while (!jobSubmissionFuture.isCompleted()) {
            try {
                ...
                try {
                    Await.result(Patterns.ask(jobClientActor,
     ...
     *                                                               execution fails.
     */
    public static JobExecutionResult submitJobAndWait(ActorSystem actorSystem, //
        ...
        JobListeningContext jobListeningContext = submitJob(actorSystem, config, //
                leaderRetrievalService, jobGraph, timeout, sysoutLogUpdates, classLoader);
     ...
     */
    public static void submitJobDetached(ActorGateway jobManagerGateway, //
                                         Configuration config, //
                                         JobGraph jobGraph, //
                                         FiniteDuration timeout, //
                                         ClassLoader classLoader//
        ...
        Object result;
        try {
            Future<Object> future = jobManagerGateway.ask(new JobManagerMessages.SubmitJob(jobGraph, ListeningBehaviour.DETACHED // only receive the Acknowledge for the job submission message
            ), timeout);
